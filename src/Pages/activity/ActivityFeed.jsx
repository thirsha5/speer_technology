...
const ActivityFeed = () => {
  ...
  const archiveAllCalls = () => {    
    ...
    for (let i = 0; i < archive.length; i++) {
      axios
        .post(url + "activities/" + archive[i].id, { is_archived: true })
        .then((response) => {});
    }
    resetAllCalls();
  };
  ...
  if (activity) {
    unArchiveItems = activity.filter((items) => {
  ...
  return (
    <Card>
      <Card.Body>
        <Card className="card" onClick={archiveAllCalls}>
          <Card.Body>Archive all Calls</Card.Body>
        </Card>
        {unArchiveItems.map((actVal) => {
          return (
            <div
              key={actVal.id}
              id={actVal.id}
              onClick={() => goToActivityDetail(actVal.id)}
            >
              -------------------
              {new Date(actVal.created_at).toLocaleDateString()}
              ------------------
              <div className="card">
                <Row>
                  <Col>
                    <div className="col-12">
                      <div className="small-box bg-light">
                        <div className="inner">
                          <h3>{actVal.from}</h3>
                          <p> tried to call on {actVal.via}</p>
                        </div>
                        <div className="icon">
